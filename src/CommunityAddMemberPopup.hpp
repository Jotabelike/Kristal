...
protected:
    ...
    bool init(std::string commId, std::string ownerId) {
        ...
        this->setTitle("Invite Member", "bigFont.fnt", 0.6f);

        auto bgSize = m_mainLayer->getContentSize();

        m_inputId = TextInput::create(200.0f, "User AccountID...", "chatFont.fnt");
        m_inputId->setPosition({ bgSize.width / 2, bgSize.height / 2 + 10.0f });
        m_inputId->getInputNode()->setAllowedChars("0123456789");
        m_mainLayer->addChild(m_inputId);

        auto btnSpr = ButtonSprite::create("Invite", "bigFont.fnt", "GJ_button_01.png", 0.8f);
        auto btn = CCMenuItemSpriteExtra::create(btnSpr, this, menu_selector(CommunityAddMemberPopup::onAdd));
        auto menu = CCMenu::create();
        menu->addChild(btn);
        menu->setPosition({ bgSize.width / 2, bgSize.height / 2 - 35.0f });
        m_mainLayer->addChild(menu);

        m_net = CommunityNetwork::create();
        m_net->retain();
        m_net->setOnInviteSent([this](const std::string& msg) {
    ...
    void onAdd(CCObject*) {
        ...
        m_net->invitarMiembroComunidad(m_ownerId, m_communityId, target);
    }
